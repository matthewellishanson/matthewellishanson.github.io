...
var ReadAlong = {
    ...
    changePhotos: function(current_index) {
        console.log(current_index);
        if (current_index > 40) {
            $('#joseph').show(1500);
        } 
        if (current_index > 75) {
            $('#joseph').fadeOut(100, function() {
                var img = $("<img id='mary' src='img/mary.jpg' class='img-responsive margin'>").hide();
            $(this).replaceWith(img);
            $('#mary').fadeIn(1500);
            });     
        }
        if (current_index > 99) {
            $('#mary').fadeOut(100, function() {
                var img = $("<img id='nativity' src='img/nativity.gif' class='img-responsive margin'>").hide();
            $(this).replaceWith(img);
            $('#nativity').fadeIn(1500);
            }); 
        }
        if (current_index > 150) {
            $('#nativity').fadeOut(100, function() {
                var img = $("<img id='shepherds' src='img/shepherds.gif' class='img-responsive margin'>").hide();
            $(this).replaceWith(img);
            $('#shepherds').fadeIn(1500);
            }); 
        }
        if (current_index > 310) {
            $('#shepherds').hide(100, function() {
                var img = $("<img id='shepherds2' src='img/shepherds2.jpg' class='img-responsive margin'>").hide();
            $(this).replaceWith(img);
            $('#shepherds2').fadeIn(1500);
            }); 
        }
    },

        /**if (current_index < 125 && current_index > 25) {
            document.getElementById('lake').style.display = 'block';
        } else {
            document.getElementById('lake').style.display = 'none';
        }
        if (current_index < 170 && current_index > 125) {
    ...
    selectCurrentWord: function() {
        ...
        /* Inserting function to change photo by passing current word index */
        this.changePhotos(current_word.index);
